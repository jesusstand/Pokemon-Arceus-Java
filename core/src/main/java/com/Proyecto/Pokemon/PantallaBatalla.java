...
public class PantallaBatalla implements Screen {
    ...
    private void cargarAssets() {
        ...
        com.badlogic.gdx.graphics.Pixmap pixmapBlanco = new com.badlogic.gdx.graphics.Pixmap(1, 1, com.badlogic.gdx.graphics.Pixmap.Format.RGBA8888);
    ...
    private static final float DURACION_FRAME = 0.3f; // Duración de cada frame en segundos (más lento para mejor apreciación)
    ...
    private void dibujarEscenaBatalla() {
        ...
        String textoJugador = pokemonJugador.getNombre() + " PS: " + pokemonJugador.getVida() + "/" + pokemonJugador.getVidaMaxima();
        ...
        String textoRival = pokemonRival.getNombre() + " PS: " + pokemonRival.getVida() + "/" + pokemonRival.getVidaMaxima();
    ...
    private void dibujarMenuAtaques() {
        ...
        float altoMenu = itemH * (ataquesDisponibles.size() + 1) + paddingY * 2; // Alto basado en cantidad de items + título
        ...
        // Borde superior
        game.batch.draw(texturaBlanca, menuX - paddingX, menuY - paddingY + altoMenu - grosorBorde, anchoMenu, grosorBorde);
        // Borde inferior
        ...
        // Borde derecho
        game.batch.draw(texturaBlanca, menuX - paddingX + anchoMenu - grosorBorde, menuY - paddingY, grosorBorde, altoMenu);
        ...
        for (int i = 0; i < ataquesDisponibles.size(); i++) {
            ...
            Color color = (i == ataqueSeleccionado) ? new Color(1f, 0.84f, 0f, 1f) : Color.BLACK; // Amarillo más oscuro para mejor contraste sobre blanco
    ...
    private void dibujarMenuMochila() {
        ...
        if (pokebolasDisponibles.isEmpty()) {
            // Calcular dimensiones para el mensaje vacío
            float anchoMenu = 250f;
            float altoMenu = itemH + paddingY * 2;
        ...
        float altoMenu = itemH * (pokebolasDisponibles.size() + 1) + paddingY * 2; // Alto basado en cantidad de items + título
        ...
        for (int i = 0; i < pokebolasDisponibles.size(); i++) {
            ...
            Color color = (i == pokebolaSeleccionada) ? new Color(1f, 0.84f, 0f, 1f) : Color.BLACK; // Amarillo más oscuro para mejor contraste sobre blanco
            fontPequeña.setColor(color);
